 ...
 * index, leftWindow contains all turns from all speakers other than the speaker at the current index
 * 
 * @author mpurver
 */
public class OtherSpeakerAllOtherTurnWindower extends TurnWindower {

	private HashMap<DialogueSpeaker, ArrayList<DialogueTurn>> speakerTurns;

	public OtherSpeakerAllOtherTurnWindower(Dialogue dialogue, int left, int right, int step) {
	...
	private void init() {
		...
		if (getDialogue() != null) {
			...
			for (DialogueTurn turn : getDialogue().getTurns()) {
				for (DialogueSpeaker speaker : getDialogue().getSpeakers()) {
					if (!speaker.equals(turn.getSpeaker())) {
						speakerTurns.get(speaker).add(turn);
					}
				}
			}
		}
