...
/**
 * For a given sentence: 1 if the last occurrence of the specified phenomenon matches this one in form AND was spoken by
 * a different speaker, 0 if it doesn't or doesn't exist. (So not a pairwise {@link SimilarityMeasure} at all really -
 * one argument will be null)
 * 
 * @author mpurver
 */
public class SentenceLastConstructionOtherSimilarityMeasure implements SimilarityMeasure<DialogueSentence> {
	 ...
	 */
	public SentenceLastConstructionOtherSimilarityMeasure(String bnfPattern1, String bnfPattern2) {
		super();
	...
	public double similarity(DialogueSentence a, DialogueSentence b) {
		...
		double score = 0.0;
		String speaker = b.getSpeaker().getId();
		for (String bnf : getBnfList(b.getSyntax())) {
			...
			if (bnf.matches(bnfPattern1)) {
				...
				if ((lastMatchingForm == 1) && (!lastSpeaker.equals(speaker))) {
				...
				if ((lastMatchingForm == 2) && (!lastSpeaker.equals(speaker))) {
